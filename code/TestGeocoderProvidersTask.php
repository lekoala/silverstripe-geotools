...
class TestGeocoderProvidersTask extends BuildTask
{
    ...
    function run($request)
    {
        ...
        if ($geo) {
            DB::alteration_message('Simple geocoding is working '.$geo['Latitude'].'/'.$geo['Longitude'],
                'created');
        } else {
        ...
        foreach ($geocoder->getProviders() as $name => $provider) {
            ...
            try {
                ...
                if ($res->count()) {
                    DB::alteration_message("Provider $name is working and returned ".$res->count().' results',
                        'created');
                    foreach ($res as $result) {
                        DB::alteration_message('Geoloc is working '.$result->getLatitude().'/'.$result->getLongitude(),
                            'created');
                    }
                } else {
