...
class GeoExtension extends DataExtension {
	 ...
	 */
	function getCountry() {
		return new \Geocoder\Model\Country($this->owner->CountryName, $this->owner->CountryCode);
	}
	...
	public function getAddressFields($geoDefaults = true) {
		...
		$postcode = new TextField('PostalCode', _t('GeoMemberExtension.POSTCODE', 'Postal Code'), $postalCodeValue);
	...
	public function ReverseGeocode() {
		...
		try {
			$results = Geocoder::getGeocoder()->reverse($this->owner->Latitude, $this->owner->Longitude);
			if ($results->count()) {
